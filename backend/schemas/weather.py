...
class CurrentWeatherResponse(BaseModel):
    ...
    source: str
    source_identity: str = Field(default="OpenWeather", description="Exact provider identity that generated the lead observation (e.g. OpenWeather, Open-Meteo)")
...
class ForecastResponse(BaseModel):
    ...
    source_identity: str = Field(default="OpenWeather", description="Exact provider identity that generated the forecast (e.g. OpenWeather, Open-Meteo)")
    units: WeatherUnitsSchema = Field(default_factory=WeatherUnitsSchema)
    issued_at: str = Field(description="Forecast issuance ISO 8601 UTC timestamp")
    retrieved_at: str = Field(description="Data retrieval ISO 8601 UTC timestamp")
    system_state: Optional[str] = Field(default="ONLINE", description="System state: ONLINE, DEGRADED, OFFLINE, DATA_STALE, SERVICE_UNAVAILABLE")
...
class AlertResponse(BaseModel):
    ...
    source_identity: str = Field(default="IMD", description="Exact provider or system identity that generated the alert response")
...
class AirQualityResponse(BaseModel):
    ...
    source: str = Field(default="OpenWeather", description="Source provider attribution")
    source_identity: str = Field(default="OpenWeather", description="Exact provider identity that generated the air quality data (e.g. OpenWeather, Open-Meteo, CPCB_MANUAL)")
    ...
    status: Optional[str] = Field(default="HEALTHY", description="Operational status: HEALTHY, DEGRADED, UNAVAILABLE, LIVE, AGING, STALE")
    freshness: Optional[str] = Field(default=None, description="Deterministic freshness classification: LIVE, AGING, STALE, UNAVAILABLE")
    is_real_time: Optional[bool] = Field(default=None, description="True only if fresh and recent observation (<15m)")
    observed_at: Optional[str] = Field(default=None, description="Observation or export timestamp")
